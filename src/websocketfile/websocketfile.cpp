...
AsyncWebSocket *ptrWebsocket = &websocket;  //set the pointer so other files can use this object
...
void processSystemStatus(JsonObject incomingJsonObjNested){
  ...
  if (!(incomingJsonObjNested["RX_LED"].isNull())){digitalWrite(SERVERBASE_RX_LED,!incomingJsonObjNested["RX_LED"].as<bool>());}
  if (!(incomingJsonObjNested["TX_LED"].isNull())){digitalWrite(SERVERBASE_TX_LED,!incomingJsonObjNested["TX_LED"].as<bool>());}
  ptrWebsocket->printfAll("%s", systemStatusJson().c_str());
}
...
void processSystemConfig(JsonObject incomingJsonObjNested){
  ...
  ptrWebsocket->printfAll("%s", systemConfigJson().c_str());
}

//-------------------------------------------------------------------
void processSchedule(JsonObject incomingJsonObjNested){
  if (!(incomingJsonObjNested["isOn"].isNull())){isScheduleOn = (incomingJsonObjNested["isOn"].as<bool>());}
  ...
  ptrWebsocket->printfAll("%s", scheduleJson().c_str());
}

//-------------------------------------------------------------------
void processMQTT(JsonObject incomingJsonObjNested){
  int rSize = 0;
  if (!(incomingJsonObjNested["Broker"].isNull())){setMQTT_BROKER(incomingJsonObjNested["Broker"].as<char*>());}
  if (!(incomingJsonObjNested["Port"].isNull())){setMQTT_PORT(incomingJsonObjNested["Port"].as<int>());}
  if (!(incomingJsonObjNested["QoS"].isNull())){setMQTT_QOS(incomingJsonObjNested["QoS"].as<int>());}
  if (!(incomingJsonObjNested["Location"].isNull())){setMQTT_LOCATION(incomingJsonObjNested["Location"].as<char*>());}
  ptrWebsocket->printfAll("%s", mqttJson().c_str());
}
